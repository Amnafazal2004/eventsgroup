...
const PastEvents: React.FC<PastEventsProps> = ({nextEventPage = false}) => {
  const [selectedRoom, setSelectedRoom] = useState<PastEvents | null>(null);
  ...
  return (
    <div className="min-h-screen py-20 px-8 md:px-16">
      ...
      <motion.h2
          className={nextEventPage ?`text-3xl md:text-4xl text-center lg:text-5xl pt-15 font-bold mb-22 ${guttie.className}`: `text-3xl md:text-4xl text-center lg:text-5xl font-bold mb-22 ${guttie.className}`}
          initial={{ opacity: 0, y: -40 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true, amount: 0.9 }}
          transition={{ duration: 1, ease: "easeOut" }}
        >
        PAST EVENTS & BOARDROOMS
        </motion.h2>
     ...
     {!nextEventPage ?<Link href={"/pastevents"}>
      <div className="flex flex-col items-center pt-10">
        <button className="mt-4 py-2 bg-black font-[fahkwang] text-white font-medium border border-amber-600 rounded-3xl hover:bg-amber-600 hover:text-black transition-all duration-300 w-58 mx-auto">
